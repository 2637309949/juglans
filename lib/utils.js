...
const repo = exports
const jConfigPath = path.join(__dirname, './config.yaml')
 ...
 */
repo.merge = function (objValue, srcValue) {
  ...
  } else if (is.array(objValue) && is.array(srcValue)) {
    return objValue.concat(srcValue)
  } else if (is.array(objValue) && is.object(srcValue)) {
    return objValue.concat([srcValue])
  } else {
...
repo.genJWT = function ({data, tokenExp, jwtSecret}) {
  try {
    return jwt.sign(data, jwtSecret, { expiresIn: tokenExp })
  } catch (error) {
