...
function Juglans (cfg = {}, options = {}) {
  ...
  assert(is.object(options), 'options entity should be a object')
  ...
  Object.defineProperty(this, 'config', {
    ...
    enumerable: true,
    writable: true,
    value: { port: 3001, name: 'Juglans V1.0', debug: true }
  })
  // default global config, injects, middles
...
Juglans.prototype.Inject = function (...params) {
  ...
  injects.reduce((acc, curr) => {
    Object.keys(curr).forEach(k => {
      const index = acc.indexOf(k)
      if (index !== -1) {
        console.warn(`key[Inject]:${k} has existed, about to be covered.`)
      }
      acc = acc.concat([k])
    })
    return acc
