...
AuthContext.prototype.createToken = async function (data) {
  try {
    ...
    const secretData = utils.auth.genJWT({data, expiresIn, secret})
...
AuthContext.prototype.findToken = async function (token) {
  try {
    ...
    if (!data) return null
    data.secret = utils.auth.parseJWT(data.secret, secret)
    return data
  } catch (error) {
 ...
 */
function getFakeUrls (config) {
  const { security: { fakeUrls, login } } = config
  return fakeUrls.concat([login])
}
 ...
 * @param {Object} param1
 */
function fakeCheck (ctx, { prefix, fakeTokens, fakeUrls, accessToken }) {
...
AuthContext.prototype.auth = function () {
  ...
  const {prefix, security: { fakeTokens }} = config
  const fakeUrls = getFakeUrls(config)
  return async function (ctx, next) {
    try {
      const accessToken = utils.auth.getTokenFromReq(ctx)
      const { isFakeTokens, isFakeUrls } = fakeCheck(ctx, { prefix, fakeTokens, fakeUrls, accessToken })
      // 非检测链接
      if (isFakeUrls) {
        ctx.state.token = {
          fakeUrl: true
