...
const routerMiddles = function (config, { login, auth, logout } = {}) {
  const {prefix, security, bodyParser = constants.KOABODYDEFULTOPTS} = config
  const router = koaRouter({prefix})
  if (login && logout) {
    router.post(security.login, login)
    router.post(security.logout, logout)
  }
  const KoaParser = koaBody(bodyParser)
  const baseMiddle = auth ? [KoaParser, auth] : [KoaParser]
