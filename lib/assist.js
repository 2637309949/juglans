...
repo._initParams = function (params) {
  ...
  // MiddleContext
  _this.MiddleContext = MiddleContext()
}
...
repo._estabApp = function (cb) {
  ...
  if (loginCheck && storeFunc) {
    ...
    middles = _this.MiddleContext.getRouterMiddles({prefix}, {login, auth, logout}, { koaBodyOpts })
    _this.MiddleContext.appendMiddle(middles)
  } else {
    middles = _this.MiddleContext.getRouterMiddles({prefix})
    _this.MiddleContext.appendMiddle(middles)
  }

  const router = middles.Router

  _this.InjectContext.appendInject({ router })
  _this._estabFunc()
  if (is.function(_this.storeFunc)) {
    _this.model = _this.storeFunc({config, mongoose})
  }

  middles = _this.MiddleContext.getMiddle()
...
repo._estabFunc = function () {
  const _this = this
  const config = _this.ConfigContext.getConfig()
  const injectParams = _this.InjectContext.getInject()
