...
function Juglans (conf = {}, opts = {}) {
  ...
  const {httpProxy, router, grpcProxy} = opts
  ...
  this.Clear()
    ...
    .PreUse(plugins.Starting, plugins.HttpProxy(httpProxy), plugins.GrpcProxy(grpcProxy), plugins.HttpRouter(router), plugins.Recovery)
    .Use(...[])
...
Juglans.prototype.ExecWithBooting = async function (b) {
  try {
    const _this = this
    this.PostUse(b)
    await runPlugins(this.prePlugins
      .Append(this.plugins)
      .Append(this.scanPlugins)
      .Append(this.postPlugins),
    () => this.injects,
    {
      execAfter (ret) {
        _this.Inject(ret)
